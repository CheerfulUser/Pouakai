...
#defines a global variable for packing numbers
SYMBOLS = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"   

def pack_number(number):
    index = number // 10
    if index > len(SYMBOLS) - 1:
        print("Cycle count too big to pack; Limit = 620")
        return number
    return SYMBOLS[index] + str(number % 10)
...
def pack_MP_number(MP_number):
    ...
    """
    if not MP_number.isdigit():
        return

    MP_number = int(MP_number)

    if MP_number >= 62 ** 4 + 620000:
        print("MP number too high to pack")
        return
    
    if MP_number < 100000:
        formatted_number = str(MP_number).rjust(5, "0")
    elif MP_number < 620000:
        formatted_number = SYMBOLS[MP_number // 10000]
        formatted_number += str(MP_number % 10000).rjust(4, "0")
    else:
        MP_number -= 620000
        formatted_number = ""
        while MP_number > 0:
            formatted_number += SYMBOLS[MP_number % 62]
            MP_number //= 62
        formatted_number = formatted_number[::-1]
        formatted_number = "~" + formatted_number.rjust(4, "0")
...
def MPC_maker(file_path):
    ...
    Parameters:
    file_path (str): The path to the FITS file to be processed.

    Steps:
    1. Extract header information from the specified FITS file.
    2. Parse the information to retrieve the list of solar system objects in the field of view (FoV).
    3. Compare this list with the source list from Pouakai.
    4. If matches are found, reformat all gathered information into a file for the MPC 
       which is saved in the same place as the fits table.
    ...
    try:
        ...
        tolerance = 0.0027778   
        #A good tolerance to use is 10 arcseconds (which is equal to 0.0027778 degrees)
        ...
        print(f"An error occurred while trying to make the MPC file: {e}")